...
class _AnalyticsDashboardPageState
  ...
  Widget topList(String title, List<MapEntry<String, int>> items, IconData icon) {
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return ListView(
              ...
              children: [
                ...
                card("المستخدمين النشطين الآن", 0,
                    Icons.online_prediction),
                const SizedBox(height: 15),

                card("عدد عمليات الدفع", 0,
                    Icons.attach_money),
                const SizedBox(height: 15),

                card("مشاهدات الكورسات", 0,
                    Icons.school),
                const SizedBox(height: 15),
          ...
          return FutureBuilder(
            ...
            builder: (context, snap2) {
              ...
              final courseSnap = snap2.data![0] as QuerySnapshot;
              final userSnap = snap2.data![1] as QuerySnapshot;
              final paymentSnap = snap2.data![2] as QuerySnapshot;
              ...
              topCourses = topCourses.map((e) {
                return MapEntry(
                    courseNames[e.key] ?? e.key, e.value);
              }).toList();

              topUsers = topUsers.map((e) {
                return MapEntry(
                    userNames[e.key] ?? e.key, e.value);
              }).toList();
              ...
              return ListView(
                ...
                children: [
                  ...
                  card("عدد عمليات الدفع", payments,
                      Icons.attach_money),
                  const SizedBox(height: 15),
                  ...
                  card("مشاهدات الكورسات", courseViews,
                      Icons.school),
                  const SizedBox(height: 15),

                  card("فتح الدروس", lessons,
                      Icons.play_circle),
                  const SizedBox(height: 25),
